...
MAX_LEN_SENTENCE = 128

"""====================== METHODS DEFINITIONS ======================"""

def truncating_padding_sentence(tokens, block_size, pad_token="<pad>"):
    if (len(tokens) > block_size):
        original_tokens_len = block_size
        tokens = tokens[:block_size]
    else:
        original_tokens_len = len(tokens)
        tokens = tokens + [pad_token]*(block_size - len(tokens))
    return tokens, original_tokens_len

class TextDataset(Dataset):
    def __init__(self, tokenizer, file_path, args, pad_token):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            for i, record in tqdm(data_df.iterrows(), total=len(data_df)):
                ...
                decoder_input, _ = truncating_padding_sentence(decoder_input, args.block_size, pad_token)
                decoder_input = tokenizer.convert_tokens_to_ids(decoder_input)
                decoder_input = np.array(decoder_input)
                # decoder_output
                decoder_label = sentence_tokenized + [tokenizer.eos_token]
                decoder_label, _ = truncating_padding_sentence(decoder_label, args.block_size, pad_token)
        ...
        logger.debug("Examples of samples from training set:")
        for i in range(2):
            example = self.examples[i]
            logger.debug("decoder_input: " + str(example["decoder_input"]))
            logger.debug("decoder_label: " + str(example["decoder_label"]))
            logger.debug("decoder_input: " + str(tokenizer.decode(example["decoder_input"].tolist(), clean_up_tokenization_spaces=True)))
            logger.debug("decoder_label: " + str(tokenizer.decode(example["decoder_label"].tolist(), clean_up_tokenization_spaces=True)))
            logger.debug("\n")
...
def load_and_cache_examples(args, file_path, tokenizer, pad_token):
    dataset = TextDataset(tokenizer, file_path=file_path, args=args, pad_token=pad_token)
    return dataset

def collate_text_batch(features, pad_token_id):
    sentence_embedding = torch.tensor(np.stack([f["sentence_embedding"] for f in features]), dtype=torch.float)
    decoder_input = torch.tensor(np.stack([f["decoder_input"] for f in features]), dtype=torch.long)
    decoder_label = torch.tensor(np.stack([f["decoder_label"] for f in features]), dtype=torch.long)

    non_pad = decoder_label.ne(pad_token_id)
    sequence_lengths = non_pad.sum(dim=1).clamp(min=1)
    max_length = int(sequence_lengths.max().item())

    decoder_input = decoder_input[:, :max_length].contiguous()
    decoder_label = decoder_label[:, :max_length].contiguous()
    decoder_attention_mask = torch.cat(
        [
            torch.ones((decoder_input.size(0), 1), dtype=torch.long),
            decoder_input.ne(pad_token_id).long(),
        ],
        dim=1,
    )
...
def save_checkpoint(model, args, loss_reports, global_step):
    ...
    model.decoder.save_pretrained(output_dir_currentstep) # save lora peft
    transform_matrix_dir = os.path.join(output_dir_currentstep, 'transform_matrix')
    if not os.path.exists(transform_matrix_dir):
        os.makedirs(transform_matrix_dir)
    torch.save(model.transform_matrix.state_dict(), os.path.join(transform_matrix_dir, 'transform_matrix.weights'))
    ...
    model.save_loggings(args, output_dir_basemodel, loss_reports)


"""====================== TRAIN/EVALUATE FUNCTION ======================"""

# train and evaluate function
def train(args, train_dataset, eval_dataset, model, tokenizer, accelerator):
    """ Train the model """
    ...
    train_dataloader = DataLoader(
        ...
        collate_fn=lambda features: collate_text_batch(features, accelerator.unwrap_model(model).pad_token_id),
        num_workers=4,
        pin_memory=True,
    )
    ...
    model.zero_grad()
    train_iterator = trange(int(args.num_train_epochs), desc="Epoch", disable=not accelerator.is_local_main_process)
    ...
    eval_current_step = []
    pad_token_id = accelerator.unwrap_model(model).pad_token_id
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # compute loss
            NLL_loss = loss_fn(decoder_lm_logits, decoder_label, pad_token_id)
            loss = NLL_loss
            ...
            loss_report.append(loss.data.cpu().float().numpy())
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.evaluate_during_training:
                        model.eval()
                        eval_loss, eval_perplexity = evaluate(args, eval_dataset, model, tokenizer, accelerator)
                    ...
                    if accelerator.is_main_process:
                        loss_reports = {"loss_report":loss_report,
                                        "eval_loss_report":eval_loss_report, "eval_perplexity_loss_report":eval_perplexity_loss_report, "eval_current_step":eval_current_step
                                        }
                        save_checkpoint(accelerator.unwrap_model(model), args, loss_reports, global_step)
    ...
    loss_reports = {"loss_report":loss_report,
                    "eval_loss_report":eval_loss_report,
                    "eval_perplexity_loss_report":eval_perplexity_loss_report,
                    "eval_current_step":eval_current_step
                    }
...
def evaluate(args, eval_dataset, model, tokenizer, accelerator):
    ...
    for step, batch in enumerate(tqdm(eval_dataloader, desc="Iteration", disable=not accelerator.is_local_main_process)):
        ...
        with torch.no_grad():
            ...
            loss_report.extend(loss.data.cpu().float().numpy())
            perplexity_report.extend(perplexity.data.cpu().float().numpy())
...
def main():
    ...
    parser.add_argument("--mixed_precision", default="no", choices=["no", "fp16", "bf16"],
                        help="Mixed precision mode used by Accelerate.")
    parser.add_argument("--gradient_checkpointing", action="store_true",
                        help="Enable decoder gradient checkpointing to reduce activation memory.")
    ...
    accelerator = Accelerator(cpu=args.no_cuda, kwargs_handlers=[ddp_kwargs], mixed_precision="bf16")
    args.device = accelerator.device
    args.n_gpu = 1 if torch.cuda.is_available() and not args.no_cuda else 0
    # setup logging
    ...
    if args.block_size <= 0:  # modify args.block_size variable
        args.block_size = min(model.tokenizer.max_len_single_sentence, MAX_LEN_SENTENCE)  # Our input block size will be the max possible for the model
    args.block_size = min(args.block_size, model.tokenizer.max_len_single_sentence - 1)
    ...
    if args.gradient_checkpointing:
        ...
        logger.info("Enabled decoder gradient checkpointing")
    ...
    if args.do_train:
        ...
        train_dataset = load_and_cache_examples(args, args.train_data_file, model.tokenizer, model.pad_token)
        if args.evaluate_during_training:
            eval_dataset = load_and_cache_examples(args, args.eval_data_file, model.tokenizer, model.pad_token)
        else:
            eval_dataset = None
        ...
        train(args, train_dataset, eval_dataset, model, model.tokenizer, accelerator)
